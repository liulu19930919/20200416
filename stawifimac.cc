...
StaWifiMac::SendAssociationRequest (bool isReassoc)
{
    ...
    std::cout << "send ass to " << GetBssid() << std::endl;
    //The standard is not clear on the correct queue for management
    //frames if we are a QoS AP. The approach taken here is to always
    //use the non-QoS for these regardless of whether we have a QoS
    //association or not.
...
void
StaWifiMac::MinChannelTimeOut(void)
{
    if (! activityDetected)
    {
...
StaWifiMac::ProbeRequestTimeout(void)
{
    ...
    if (m_bestAp.m_channelNumber ！= 0)
    {
        NS_LOG_DEBUG ("Attempting to associate with BSSID " << m_bestAp.m_bssid);
        Time beaconInterval;
        if (m_bestAp.m_activeProbing)
        {
            UpdateApInfoFromProbeResp (m_bestAp.m_probeResp, m_bestAp.m_apAddr, m_bestAp.m_bssid);
            beaconInterval = MicroSeconds (m_bestAp.m_probeResp.GetBeaconIntervalUs ());
        }
        else
        {
            UpdateApInfoFromBeacon (m_bestAp.m_beacon, m_bestAp.m_apAddr, m_bestAp.m_bssid);
            beaconInterval = MicroSeconds (m_bestAp.m_beacon.GetBeaconIntervalUs ());
        }
...
StaWifiMac::Receive (Ptr<Packet> packet, const WifiMacHeader *hdr)
{
    ...
    else if (hdr->IsBeacon ())
    {
        ...
        if (goodBeacon && m_state == WAIT_BEACON)
        {
            ...
            SnrTag snrTag;
            if (m_bestAp.m_snr < tag.Get ())
            {
                NS_LOG_DEBUG ("Beacon has highest SNR so far: " << tag.Get ());
                m_bestAp.m_channelNumber = m_phy->GetChannelNumber ();
                m_bestAp.m_snr = tag.Get ();
                m_bestAp.m_bssid = hdr->GetAddr3 ();
                m_bestAp.m_apAddr = hdr->GetAddr2();
                m_bestAp.m_activeProbing = false;
                m_bestAp.m_beacon = beacon;
            }
        }
        return;
    }
    else if (hdr->IsProbeResp ())
    {
        if (m_state == WAIT_PROBE_RESP)
        {
            ...
            // 更新3
            std::cout << "Probe response received while scanning from " << hdr->GetAddr2 () << std::endl;
            ...
            if (m_bestAp.m_snr < tag.Get ())
            {
                ...
                m_bestAp.m_bssid = hdr->GetAddr3 ();
                m_bestAp.m_apAddr = hdr->GetAddr2();
                m_bestAp.m_activeProbing = true;
                m_bestAp.m_probeResp = probeResp;
            }
        }
        return;
    }
